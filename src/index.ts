...
server.addHook('onSend', async (request, reply, payload) => {
  ...
  console.log(`[${new Date().toISOString()}] ${method} ${url} (ID: ${id}) -> ${reply.statusCode}`);
  if (payload) {
...
const start = async () => {
  ...
  try {
    ...
    try {
      await fs.promises.mkdir(path.join(storagePath, 'avatars'), { recursive: true });
    ...
    server.get('/storage/avatars/:filename', async (request: any, reply) => {
    ...
    while (attempts < MAX_RETRIES && !serverStarted) {
      ...
      try {
        ...
        if (attempts >= MAX_RETRIES) {
          console.error(`Failed to start server after ${MAX_RETRIES} attempts:`, err);
          throw err;
        }
